...
- интерактивная карта объектов на Яндекс Картах;
...
Для работы карты во фронтенде нужен ключ Яндекс Карт:

```bash
VITE_YANDEX_MAPS_API_KEY=ваш_ключ
```

Локальный frontend настроен на `http://127.0.0.1:5173`. В ограничениях ключа Яндекс Карт для разработки нужно разрешить host `127.0.0.1`.
